...
async function handleVote(type, postId = null, commentId = null) {
    ...
    try {
        ...
        if (data.success) {
            ...
            function changeLikeCounts(className, newCount) {
                const targets = document.querySelectorAll(`.${className}`);
                targets.forEach((target) => {
                    if (target.getAttribute('data-post-id') === postId) {
                        // console.log("Found element:", target)
                        target.querySelector('.span-x').innerText =  String(newCount);
                        // target.innerText = String(data.likeCount);
                    }
                })

            }

            changeLikeCounts('like-btn', data.likeCount)
            changeLikeCounts('dislike-btn', data.dislikeCount)
